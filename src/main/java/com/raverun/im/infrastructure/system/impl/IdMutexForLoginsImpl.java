...
/**
 * FIXME a temporary duplicate of {@link IdMutexProvider}
 */
public class IdMutexForLoginsImpl implements IdMutexForLogins
{
    ...
    public Mutex getMutex( String id )
    {
        ...
        synchronized( mutexMap )
        {
            ...
            if( ref == null )
            {
                ...
                _logger.debug( "Create MUTEX. key( " + id + ") missing in cache. " );
                return key;
            }
            Mutex mutex = ref.get();
            if( mutex == null )
            {
                mutexMap.put( key, new WeakReference<Mutex>( key ) );
                _logger.debug( "Create MUTEX. Mutex for key( " + id + ") was GC'ed." );
                return key;
            }
            return mutex;
        }
    ...
    private static final Logger _logger = Logger.getLogger( IdMutexForLoginsImpl.class );
}
